...
def refresh_dashboard_data():
    ...
    try:
        ...
        # Standardize and format stats using utility functions
        ...
        if not is_valid:
            logger.warning(f"Stats validation failed in refresh_dashboard_data: missing {missing_keys}")
...
def index():
    ...
    recent_inventory = []
    # Initialize stats using utility function for consistency
    ...
    try:
        ...
        try:
            ...
            recent_creditors = recent_debtors = recent_payments = recent_receipts = recent_inventory = []
        ...
        try:
            ...
            # Use utility function to ensure stats has safe defaults
            fallback_stats = {
                'total_debtors': len(recent_debtors),
                'total_creditors': len(recent_creditors),
                'total_payments': len(recent_payments),
            ...
            stats = utils.standardize_stats_dictionary(fallback_stats, log_defaults=True)
        ...
        # Standardize stats dictionary using utility function
        standardized_stats = utils.standardize_stats_dictionary(stats, log_defaults=True)
        
        # Validate stats completeness for debugging
        is_valid, missing_keys, warnings = utils.validate_stats_completeness(standardized_stats, 'dashboard_index')
        if not is_valid:
            logger.warning(f"Stats validation failed in dashboard index: missing {missing_keys}")
        if warnings:
            logger.info(f"Stats validation warnings in dashboard index: {warnings}")

        # Render dashboard with standardized stats
        ...
        # Use utility function to ensure safe stats even in critical error scenarios
